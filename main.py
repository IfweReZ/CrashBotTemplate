...
bot = commands.Bot(command_prefix="hz", intents=dildo.Intents.all())
...
async def on_ready():
    global genid2
    activity = dildo.Streaming(
        name=cfg.Settings.ActivityText,
        url=cfg.Settings.ActivityURL, type=3)
    await bot.change_presence(status=dildo.Status.dnd, activity=activity)
    now = datetime.now()
    discord = Colorate.Horizontal(Colors.rainbow, text='ifwerez')
...
░ ▓██▓ ░ ▒▓█  ▄ ▒██    ▒██ ▒██▄█▓▒ ▒▒██░   ░██▄▄▄▄██░ ▓██▓ ░ ▒▓█  ▄       
  ...
  ░         ░   ░      ░   ░░         ░ ░    ░   ▒    ░         ░         
            ░  ░       ░                ░  ░     ░  ░           ░  ░"""))
    print(f"""                                               {Style.BRIGHT}{Fore.RESET}My Discord is {discord}
...
async def on_guild_channel_create(channel):
    for i in range(cfg.Settings.SpamAmount):
        ...
        except dildo.errors.HTTPException:
            now = datetime.now()
            print(
...
async def kickall(inter, nokick: dildo.User):
